...
import {
  ThermalInstance,
  ThermalInstanceDisplayParameters,
} from "../../../instance/thermalInstance";
...
export const SingleInstanceDetail: React.FC<SingleInstanceDetailProps> = ({
  ...
  const { value: minmax } = useThermalRegistryMinmaxState(
    ...
    () => {
      return [
        ...
        TimeFormat.human(instance.timestamp)
          ...
          .replaceAll(":", "-")
          .replaceAll(" ", "_"),
        `${instance.group.registry.range.value?.from.toFixed(
          1
        )} až ${instance.group.registry.range.value?.to.toFixed(1)} °C`,
    ...
    <>
      <div className="w-full flex flex-wrap items-center gap-2">
        ...
        <Tooltip content="Stáhnout termogram" color="foreground">
          <Dropdown>
            <DropdownTrigger>
              <Button isIconOnly isDisabled={printingState.isLoading}>
            ...
            <DropdownMenu
              onAction={(key) => {
                if (key === "export") {
                  if (printingState.isLoading === false) download();
                }
              }}
            >
